...
export default function EmployeeDashboard({ user }: EmployeeDashboardProps) {
  ...
  const daysWorkedSet = new Set(
    clocksThisMonth.map(c => new Date(c.clock_in).toDateString())
  );
  ...
  return (
    <div className="container mx-auto px-4 sm:px-6 py-8">
      <div className="mb-8">
        <h2 className="text-white/90 mb-[6px] mt-[0px] mr-[0px] ml-[0px]">Welcome back, {user.first_name}!</h2>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-white/90">
                {user.team?.manager ? `${user.team.manager.first_name} ${user.team.manager.last_name}` : "N/A"}
              </span>
            </div>
            <div className="flex flex-col">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <StatCard
          ...
          trend={avgLateMinutes > 0 ? { value: `${Math.round(avgLateMinutes)} min late`, positive: false } : undefined}
        />
        <StatCard
        ...
        <StatCard
          ...
          icon={UserIcon}
          description={user.team?.manager?.first_name ? `Manager: ${user.team.manager.first_name}` : "Not assigned"}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        ...
        <div className="lg:col-span-1">
          <div className="backdrop-blur-xl bg-white/5 border border-white/10 rounded-xl overflow-hidden h-[420px] flex flex-col">
            <h3 className="text-white/80 text-sm p-4 border-b border-white/10 flex-shrink-0">Your Clock Records</h3>
              ...
              <div className="p-4 space-y-3">
                {clocks.length === 0 ? (
                  ...
                  clocks.slice(0, 5).map((clock) => {
                    ...
                    return (
                      <div key={clock.id} className="bg-white/5 rounded-lg p-3 border border-white/5 transition-all duration-500 ease-in-out animate-in fade-in slide-in-from-top-2">
                        <div className="flex justify-between items-start mb-2">
                          <span className="text-white/60 text-xs">
                            {new Date(clock.clock_in).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                          </span>
                        ...
                        <div className="text-xs text-white/70 space-y-1">
                          <div className="flex justify-between">
                            <span>In:</span>
                            <span>{new Date(clock.clock_in).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: false })}</span>
                          </div>
                          <div className="flex justify-between">
                            <span>Out:</span>
                            <span>{clock.clock_out ? new Date(clock.clock_out).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: false }) : '-'}</span>
                          </div>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between mb-3">
                          <div>
                            ...
                            <span className={`inline-flex items-center gap-1.5 text-xs px-3 py-1 rounded-full backdrop-blur-sm ${
                              isActive 
                        ...
                        <div className="space-y-2.5">
                          ...
                          <div className="flex items-center gap-3 group/item">
                            ...
                            <span className="text-white/80 text-sm">{fullUser?.phone_number || 'N/A'}</span>
                          </div>
                        </div>
                      </div>
                    </div>
