...
export default function UserPage() {
  ...
  const getTodayWorkingHours = () => {
    ...
    const todayClocks = clocks.filter((clock) => new Date(clock.clock_in).toDateString() === today)
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Check if an active clock exists
  ...
  return (
    ...
    >
      ...
      <Box
        sx={{ display: 'flex', flexDirection: 'row', gap: 4, maxWidth: '1600px', margin: '0 auto' }}
      >
        {/* --- USER CARD --- */}
        <Card
          elevation={0}
        ...
        >
          ...
          <Box
            sx={{
              background:
                'linear-gradient(145deg, rgba(59, 130, 246, 0.1), rgba(139, 92, 246, 0.1))',
              borderRadius: '50%',
              p: 2,
              mb: 3,
          ...
          <Stack direction="row" spacing={3} sx={{ mb: 4, width: '100%' }}>
            ...
            <Box sx={{ flex: 1, textAlign: 'center' }}>
              ...
              <Typography
                variant="h6"
                sx={{ color: isClockedIn ? '#10b981' : '#f59e0b', fontWeight: 600 }}
              >
          ...
          >
            {processing ? <CircularProgress size={20} sx={{ color: 'white', mr: 1 }} /> : null}
